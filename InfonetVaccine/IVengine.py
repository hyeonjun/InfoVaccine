...
class Engine :
    ...
    def __init__(self, debug=False):
        ...
        # 초기값으로는 1980-01-01을 정한다.
        self.max_datetime = datetime.datetime(1980,1,1,0,0,0,0)

        # 임시 파일 모두 제거
        self.__remove_tempfile()

    # ---------------------------------------------------------------------
    # __remove_tempfile(self)
    # 임시 폴더에 존재하는 임시 파일 제거
    # ---------------------------------------------------------------------
    def __remove_tempfile(self):
        path = tempfile.gettempdir()
        fl = glob.glob(path + os.sep + 'ktmp*')
        for name in fl:
    ...
    def set_plugins(self, plugins_path):
        ...
        # 공개키를 로딩한다.
        pu = IVrsa.read_key(plugins_path + os.sep + 'key.pkr')
        if not pu :
            return False

        # 우선순위를 알아낸다.
        ret = self.__get_kmd_list(plugins_path + os.sep + 'infonet.kmd', pu)
        ...
        # 우선 순위대로 KMD 파일을 로딩한다.
        for kmd_name in self.kmdfiles :
            kmd_path = plugins_path + os.sep + kmd_name
            k = IVkmdfile.KMD(kmd_path, pu) # 모든 KMD 파일을 복호화한다.
            module = IVkmdfile.load(kmd_path.split('.')[0], k.body)
            if module : # 메모리 로딩 성공
        ...
        for fname in fl:
            try:
                ...
                if buf[0:4] == 'KAVS':
                    sdate = IVtimelib.convert_date(struct.unpack('<H', buf[8:10])[0])
                    stime = IVtimelib.convert_time(struct.unpack('<H', buf[10:12])[0])
    ...
    # -----------------------------------------------------
    # __get_kmd_list(self, kmd_file, pu)
    # 플러그인 엔진의 로딩 우선순위를 알아낸다
    # 인자값 : kmd_file - infonet.kmd 파일의 전체 경로
    #         pu - 공개키
    # 리턴값 : 성공 여부
    # -----------------------------------------------------
    def __get_kmd_list(self, kmd_file, pu):
        kmdfiles = [] # 우선 순위 목록

        k = IVkmdfile.KMD(kmd_file, pu) # infonet.kmd 파일을 복호화한다.

        if k.body: # infonet.kmd 읽혔는지
...
class EngineInstance :
    ...
    def scan(self, filename, *callback):
        ...
        # 1. 검사 대상 리스트에 파일을 등록
        file_info = IVfile.FileStruct(filename)
                ...
                if os.path.isdir(real_name):
                    ...
                    if is_sub_dir_scan:
                        ...
                        for rfname in flist:
                            ...
                            tmp_info = IVfile.FileStruct(rfname)
                    ...
                    # 압축된 파일이면 해제하기
                    if real_name == '': # 이미 실제 파일명이 존재하지 않으면 압축 파일이다.
                    ...
                    if ret_value['result']: # 악성코드 발견했나?
                        ...
                        if scan_callback_fn:
                            ...
                            if self.options['opt_move']:
                                ...
                                if action_type == IVconst.K2_ACTION_QUIT:  # 종료인가?
                                    return 0
                                d_ret = self.disinfect_process(ret_value, action_type)

                                if d_ret: # 치료 성공?
                                    # 악성코드 치료 후 해당 파일이 삭제되지 않고 존재? 그럼 다시 검사 필요!
                                    if self.options['opt_dis'] or (action_type == IVconst.K2_ACTION_DISINFECT or action_type == IVconst.K2_ACTION_DELETE):  # 치료 옵션이 존재할때에만 실행
                                        # 치료 옵션이 존재할때에만 실행!
                                        if os.path.exists(t_file_info.get_filename()):
                                            ...
                                            # 압축 파일 최종 치료 처리
                                            self.update_process(t_file_info)
                    else:
                        # 압축 파일 최종 치료 처리
                        self.update_process(t_file_info)

                        # 이미 해당 파일이 악성코드라고 판면되었다면
                        # 그 파일을 압축해제해서 내부를 볼 필요 없다.
                        # 압축 파일이면 검사 대상 리스트에 추가한다.
                        try:
        ...
        self.update_process(None, True)  # 최종파일 정리
    ...
    # --------------------------------------------------------------------
    # update_process(self, file_struct, update_callback_fn,
    #                  immediately_flag=False)
    # update_info를 갱신한다.
    # 입력값 : file_struct         - 파일 정보 구조체
    #         update_callback_fn  - update 콜백 함수
    #         immediately_flag    - update_info 모든 정보 갱신 여부
    # ---------------------------------------------------------------------
    def update_process(self, file_struct, immediately_flag=False):
                ...
                if p_file_info.get_master_filename() == n_file_info.get_master_filename() and \
                    ...
                    if p_file_info.get_level() <= n_file_info.get_level():
                        ...
                        while ret_file_info.get_level() != n_file_info.get_level():
                            # 마스터 파일이 같고 압축 깊이가 달라지면 내부 파일 업데이트
    ...
    def __update_arc_file_struct(self, p_file_info):
        ...
        if b_update:  # 수정된 파일이 존재한다면 재압축 진행
            ...
            if ret:
                if self.update_callback_fn and ret_file_info:
    ...
    # --------------------------------------------------------------------
    # disinfect_process(self, ret_value, disinfect_callback_fn,
    #                       action_type)
    # 악성코드를 치료한다.
    ...
    # ---------------------------------------------------------------------
    def disinfect_process(self, ret_value, action_type):
        if action_type == IVconst.K2_ACTION_IGNORE: # 치료에 대해 무시
            return
        ...
        if action_type == IVconst.K2_ACTION_DISINFECT: # 치료 옵션이 설정되었나?
            d_ret = self.disinfect(d_fname, mid, eid)
            if d_ret:
                self.result['Disinfected_files'] += 1 # 치료 파일 수
        elif action_type == IVconst.K2_ACTION_DELETE: # 삭제 옵션이 설정되었나?
            try:
                os.remove(d_fname)
                d_ret = True
    ...
    def __scan_file(self, file_struct, fileformat) :
        ...
        except (EngineKnownError, ValueError, WindowsError) as e :
            pass
        except KeyboardInterrupt:
                    ...
                    try :
                        ...
                        if unpack_data :
                            ...
                            if self.options['opt_sigtool']:
                                ...
                                # sigtool.log 파일을 생성
                                msg = '%s : %s\n' % (sig_fname, rname_struct.
                                                     get_additional_filename())
    ...
    def arclist(self, file_struct, fileformat):
        ...
        # 압축 엔진 모듈의 arclist 멤버 호출 호출
        for inst in self.kavmain_inst:
            is_archive_engine = False


            # 플러그인 엔진에 getinfo()에 kernel.ARCHIVE_ENGINE이 존재하는가?
            try:
            ...
            if len(arc_list): # 압축 목록이 존재한다면 추가하고 종료
                for alist in arc_list:
                    ...
                    fs = IVfile.FileStruct()
                    # 기본 level보다 1 증가시켜 압축 깊이가 깊어짐을 표시
                    fs.set_archive(arc_id, rname, name, dname, mname,
                                    False, False, level+1)
    ...
    def format(self, file_struct):
        ...
        except (IOError, EngineKnownError, ValueError, OSError, WindowsError) as e:
            pass
    ...
    def set_options(self, options=None) :
        if options :
            self.options['opt_arc'] = options[0]
            self.options['opt_dis'] = options[1]
            self.options['opt_nor'] = options[2]
            self.options['opt_list'] = True
            self.options['opt_move'] = options[2]
            self.options['infp_path'] = options[2]
            self.options['opt_sigtool'] = options[2]
        else :
