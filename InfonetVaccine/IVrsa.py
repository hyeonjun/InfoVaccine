...
# 확장 유클리드 호제법 알고리즘
# -------------------------------------------------------------------------------
# 정수 m, n의 최대공약수(Greatest Common Divisor)를 gcd(m,n)와 나타낼 때,
...
def __mr(n) :
    ...
    for i in range(10) : # 10번의 소수 여부 테스트
...
def __gen_number(gen_bit) :
    random.seed() # seed는 random 함수의 일종의 기준
...
# 또한 확장 유클리드 호제법을 이용해서 d * e / n으로 나눴을 때 나머지가 1인
# 정수 d를 찾는다.
# 인자값 : n - 정수
# 리턴값 : e, d
# -------------------------------------------------------------------------------
...
# 인자값 : key_filename - rsa 파일
# 리턴값 : rsa 키
# ------------------------------------------------------------------------------
def read_key(key_filename) :
    try:
        with open(key_filename, 'rt') as fp:
    #   위와 아래는 같지만 with는 블록이 끝나면 자동으로 fp.close() 함수가 호출된다.
    #   fp = open(key_filename)
    #   b = fp.read()
    #   fp.close()
            b = fp.read()
