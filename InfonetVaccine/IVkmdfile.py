...
def make(src_fname, debug=False):
    ...
    # -----------------------------------------------------------------
    # 공개키를 로딩한다.
    rsa_pu = IVrsa.read_key('key.pkr')
    # print 'pkr : ', rsa_pu

    # 개인키를 로딩한다.
    rsa_pr = IVrsa.read_key('key.skr')
    # print 'skr : ', rsa_pr
    ...
    # 현재 날짜와 시간을 구한다.
    ret_date = IVtimelib.get_now_date()
    ret_time = IVtimelib.get_now_time()
    ...
    while 1:
        ...
        # 생성된 RC4 키를 암호화한다.
        e_key = IVrsa.crypt(key, rsa_pr)  # 개인키로 암호화
        if len(e_key) != 32:  # 암호화에 오류가 존재하면 다시 생성
            continue

        # 암호화된 RC4 키를 복호화한다.
        d_key = IVrsa.crypt(e_key, rsa_pu)  # 공개키로 복호화
        ...
        if key == d_key and len(key) == len(d_key):
            ...
            e_md5 = IVrsa.crypt(m, rsa_pr)  # MD5 결과를 개인키로 암호화
            if len(e_md5) != 32:  # 암호화에 오류가 존재하면 다시 생성
                continue

            d_md5 = IVrsa.crypt(e_md5, rsa_pu)  # 암호화횓 MD5를 공개키로 복호화
...
class KMD(KMDConstants):
    ...
    def __decrypt(self, fname, debug=False):
        ...
        self.date = IVtimelib.convert_date(struct.unpack('<H', tmp)[0])
        # print self.date

        # KMD 파일 시간 읽기
        tmp = self.__kmd_data[self.KMD_TIME_OFFSET:
                              self.KMD_TIME_OFFSET + self.KMD_TIME_LENGTH]
        self.time = IVtimelib.convert_time(struct.unpack('<H', tmp)[0])
        # print self.time
    ...
    def __get_rc4_key(self):
        ...
        return IVrsa.crypt(e_key, self.__rsa_pu)
    ...
    def __get_md5(self):
        ...
        return IVrsa.crypt(e_md5, self.__rsa_pu)
...
def load(mod_name, buf):
    if buf[:4] == '03F30D0A'.decode('hex'):  # pyc 시그너처가 존재하는가?
